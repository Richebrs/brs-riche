...
createApp({
  data() {
    return {
      user: null,
      form: {
        signup: { email: "", password: "", referrer: "" },
      ...
      error: "",
      DAILY_RATE: 0.02,
      REFERRAL_LEVELS: [0.15, 0.05, 0.05] // commissions par niveau
    };
  },
  computed: {
    lastAppliedAtText() {
  ...
  methods: {
    format(n) {
      return new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'USD' }).format(Number(n || 0));
    },
